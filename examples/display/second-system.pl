...
typeclass[*VRAM] => sub {
    ...
    method update => sub ($vram) {
        ...
        my $out = join '' => @out, format_goto( 0, $vram->rows / 2 + 1 );
        print $out;
...
=cut
    ...
    my @cols = 0 .. $vram->cols - 2;
    my @rows = 0 .. $vram->rows - 2;

    {
        ...
        while (1) {
            ...
            last if scalar @my_rows == 0
                 && scalar @my_cols == 0;

            my $row = shift @my_rows;
            my $col = pop @my_cols;

            draw_ball( $row, $Red )   if $row;
            draw_ball( $col, $Green ) if $col;

            $vram->update();

            sleep(0.02);
            draw_ball( $row, undef ) if $row;
            draw_ball( $col, undef ) if $col;
        }
